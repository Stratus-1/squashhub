...
- Engine capability lives in `src/lib/smart-builder/engine-support.ts` and is read by validate, Review, to-existing and specFromDefinition; exact equivalents are translated, others block at design time; why: the builder must never design what the engine can't run.
