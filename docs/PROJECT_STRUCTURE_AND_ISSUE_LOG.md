...
- **Fix:** the scorecard now resolves rules through the local team association, includes every other same-tier squad when the club's same-night option is enabled, and labels those players with their existing team. Rule saves explicitly create/update the tenant association row and validate the returned association. Nelspruit Singles League now has a tenant override with the option enabled.
