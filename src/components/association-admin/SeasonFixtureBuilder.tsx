...
import { CalendarDays, Check, ChevronLeft, ChevronRight, Loader2, Save, ShieldAlert } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { generateSeasonFixtures, type DivisionPlan, type SeasonPlanResult } from "@/lib/leagues/season-fixtures";
import { expandRange, publicHolidays, schoolBreaks, weeksInRange, type DateRange } from "@/lib/leagues/calendar";
...
export function SeasonFixtureBuilder({ tenantId, association, teams, open, onOpenChange }: Props) {
  ...
  const exclusions = useMemo(() => {
    ...
    return result;
  }, [year, breakItems, excludedBreaks, manualDates]);
  ...
  const reset = () => { setStep(1); setSeasonId(""); setSelected({}); setManualDates(""); setExcludedBreaks({}); setSaving(false); };
  ...
  return <Dialog open={open} onOpenChange={close}>
    <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
      ...
      {step === 1 && <div className="space-y-4">
        ...
        <div className="flex items-center justify-between border-b pb-2"><div><p className="text-sm font-medium">Submitted teams</p><p className="text-[11px] text-muted-foreground">Choose the teams that will participate in this season.</p></div><Button type="button" variant="outline" size="sm" onClick={() => toggleAll(scopedTeams.length !== teams.length)}>{scopedTeams.length === teams.length ? "Clear all" : "Select all"}</Button></div>
        <div className="grid gap-1 sm:grid-cols-2">{teams.map((team) => <label key={team.team_id} className="flex items-center gap-2 rounded border p-2 text-xs"><Checkbox checked={!!selected[team.team_id]} onCheckedChange={(value) => setSelected((current) => ({ ...current, [team.team_id]: !!value }))} /><span className="min-w-0 flex-1 truncate">{team.team_name}</span><span className="text-muted-foreground truncate">{team.club_name}</span></label>)}</div>
      </div>}
