              ...
              return (
                <TableRow key={inv.id}>
                  <TableCell className="font-mono text-xs">
                    <button className="text-primary hover:underline" onClick={() => openInvoice(inv)}>
                      {inv.invoice_number}
                    </button>
                  </TableCell>
    ...
    <div className="space-y-4 mt-4">
      ...
      <Tabs defaultValue="subscription" className="w-full">
        ...
        <TabsContent value="billing" className="mt-4">
          <BillingInfoTab clubId={clubId} clubName={club?.name} />
        </TabsContent>
        ...
        <TabsContent value="payments" className="mt-4">
          <Card className="p-4 space-y-3">
            ...
            {invLoading ? (
              ...
              invoiceRows(paidInvoices, "No payments recorded yet.")
            )}
          </Card>
        </TabsContent>

        <TabsContent value="audit" className="mt-4">
          <BillingAuditLog clubId={clubId} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function SubscriptionSummaryPanel({
  clubId,
  totalOutstanding,
  currency,
  outstandingCount,
  ...
  const { data: sub } = useQuery({
    queryKey: ["club-subscription-summary", clubId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("club_subscriptions")
        .select("status, current_period_end, trial_ends_at, member_count, amount_due, subscription_plans(name)")
        .eq("club_id", clubId)
        .maybeSingle();
      if (error) throw error;
      return data as any;
    ...
    <Card className="p-3 md:p-4 border-primary/30 bg-primary/5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SummaryCell
          label="Amount due"
          value={
            <span className={totalOutstanding > 0 ? "text-amber-700 dark:text-amber-300 font-bold" : "font-bold"}>
...
function BillingAuditLog({ clubId }: { clubId: string }) {
  const { data: rows = [], isLoading } = useClubBillingAudit(clubId);
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center gap-2">
        <History className="w-4 h-4 text-primary" />
        <h3 className="font-semibold text-sm">Billing Information Audit Log</h3>
      </div>
      {isLoading ? (
        <div className="flex items-center gap-2 text-sm text-muted-foreground py-8 justify-center">
        ...
        <div className="text-sm text-muted-foreground py-8 text-center border border-dashed rounded-md">
          No billing information changes recorded yet.
        </div>
      ) : (
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-xs">{BILLING_FIELD_LABELS[r.field] || r.field}</TableCell>
                  <TableCell className="text-xs text-muted-foreground max-w-[220px] truncate">
                    {r.old_value || "—"}
                  </TableCell>
                  <TableCell className="text-xs max-w-[220px] truncate">{r.new_value || "—"}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
