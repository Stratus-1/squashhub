...
export function FillUpLeaguesTab({ clubId, activeMemberId }: Props) {
  ...
  // lineup (any league with <4 players assigned). Falls back to the first candidate.
  ...
  // Pick the earliest candidate week where ANY league has < 4 players assigned.
  // If every league across every scanned week is full (unlikely), use the first candidate.
  const weekStart = useMemo(() => {
    if (candidateWeeks.length === 0) return format(new Date(), "yyyy-MM-dd");
    if (leagues.length === 0) return candidateWeeks[0];
    // Build counts: weekStart -> leagueId -> Set(memberId)
    ...
    for (const wk of candidateWeeks) {
      ...
      // Week is "incomplete" if any league has <4 filled
      const allComplete = leagues.every(lg => (lm?.get(lg.id)?.size ?? 0) >= 4);
      if (!allComplete) return wk;
    }
    return candidateWeeks[candidateWeeks.length - 1];
  }, [candidateWeeks, lookaheadLineups, leagues]);
  ...
  // Index of the chosen week (for banner copy: "next week" vs "in N weeks")
  const chosenWeekIndex = useMemo(() => candidateWeeks.indexOf(weekStart), [candidateWeeks, weekStart]);
