  ...
  const spec: TournamentSpec = {
    ...
    divisions: def.divisions.map((d) => {
      ...
      d.sections.forEach((sec) => sec.stages.forEach((st) => {
        ...
        else if (st.kind === "cross_pool_league") throw new IntegrityError("unsupported_stage", `${d.name} · ${st.name}: cross-pool league games can be previewed in the builder, but creating them in the live tournament is not switched on yet.`);
