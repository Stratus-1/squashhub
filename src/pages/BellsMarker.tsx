...
export default function BellsMarker() {
  ...
  const capMinutes = useMemo(() => {
    if (!champ) return 30;
    return BellsFormat.getTimeCapMinutes(champ, match?.group_number, match?.section_number) ?? 30;
  }, [champ, match?.group_number, match?.section_number]);
