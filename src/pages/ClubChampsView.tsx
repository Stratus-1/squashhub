...
import { Loader2, ArrowLeft, Download, FileSpreadsheet, Printer } from "lucide-react";
...
export default function ClubChampsView() {
  ...
  const { data: entries = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await fromExt("club_champs_entries")
        .select("*, club_members:club_member_id(id, name, user_id, league_player_rank, profiles:user_id(name, avatar_url))")
        .eq("champ_id", champId!);
      if (error) throw error;
      return data || [];
    },
  ...
  const { data: matches = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await fromExt("club_champs_matches")
        .select("*, player_a:player_a_member_id(id, name, profiles:user_id(name)), player_b:player_b_member_id(id, name, profiles:user_id(name)), court:court_id(name)")
        .eq("champ_id", champId!)
  ...
  const getGroupStandings = (groupNum: number) => {
    ...
    return groupEntries.map((e: any) => {
      ...
      return {
        ...
        won,
        lost,
        points: won * 2 + (played - won - lost), // 2 for win, 1 for loss played
        name: getPlayerName(e.club_members),
      };
  ...
  const exportCSV = () => {
    if (!champ) return;
    const rows = [["Date", "Time", "Court", "Group", "Player A", "Player B", "Status", "Winner", "Score"]];
    matches.forEach((m: any) => {
      rows.push([
        ...
        getPlayerName(m.player_a),
        getPlayerName(m.player_b),
        m.status,
        m.winner_member_id ? (m.winner_member_id === m.player_a_member_id ? getPlayerName(m.player_a) : getPlayerName(m.player_b)) : "",
        m.score || "",
      ]);
    });
      ...
      <div className="max-w-5xl mx-auto space-y-6 print:space-y-4">
        ...
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            {champ.gender === "men" ? "Men's" : "Ladies'"} Championship · {champ.start_date} to {champ.end_date}
          </p>
        ...
        {groupNumbers.map((gn) => {
          ...
          return (
            <Card key={gn}>
              <CardHeader>
                <CardTitle className="text-lg">Group {gn}</CardTitle>
              </CardHeader>
