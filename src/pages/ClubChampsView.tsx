...
import { withdrawnMemberIds } from "@/hooks/use-champ-withdrawn";
...
export default function ClubChampsView() {
  ...
  const getGroupStandings = (groupNum: number, poolNumber?: number | null) => {
    ...
    // Pool-scoped filtering (multiple pools per league — Swiss or round robin).
    // Players who have entered but are not yet drawn have no pool, so they are
  ...
  // ── Swiss pairing ────────────────────────────────────────────────────────
  const isSwiss = (champ as any)?.round_format === "swiss" || (champ as any)?.scoring_mode === "swiss";
  ...
  function renderAllGroups() {
    ...
    const survivorsCard = koRunning && survivorsByLeague.length > 0 ? (
      <CollapsibleCard key="survivors" defaultOpen={false} className="border-emerald-500/40 bg-emerald-50/40 dark:bg-emerald-500/5"
        title={
