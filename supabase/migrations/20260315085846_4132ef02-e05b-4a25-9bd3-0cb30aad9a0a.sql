...
-- Create a function that splits light fees among confirmed event attendees
-- Called when a light_session ends (status changes to 'completed')
...
BEGIN
  ...
  v_share := ROUND(v_fee_charged / v_confirmed_count, 2);
  ...
  LOOP
    INSERT INTO public.member_credit_transactions (
      user_id, club_id, club_member_id, amount, type, description, status, method
    ) VALUES (
      r.user_id,
      NEW.club_id,
      r.club_member_id,
...
-- Attach trigger to light_sessions
CREATE TRIGGER trg_split_event_light_fees
  AFTER UPDATE ON public.light_sessions
  FOR EACH ROW
  EXECUTE FUNCTION public.split_event_light_fees();
