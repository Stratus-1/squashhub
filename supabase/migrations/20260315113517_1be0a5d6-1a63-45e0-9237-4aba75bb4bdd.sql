...
-- Member-based match rollups (mirrors _match_rollups_for_player but uses member_id columns)
...
REVOKE ALL ON FUNCTION public._match_rollups_for_member(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._match_rollups_for_member(uuid) TO authenticated;

-- get_squash_totals_by_member: same as get_squash_totals but keyed by club_member_id
...
BEGIN
  ...
  SELECT user_id INTO target_user_id FROM public.club_members WHERE id = target_member_id;

  IF target_user_id IS NOT NULL AND uid <> target_user_id THEN
  ...
  groups AS (
    SELECT is_win, rn, sum(changed) OVER (ORDER BY rn) AS grp
    FROM ordered
  ),
  streaks AS (
...
BEGIN
  ...
  RETURN QUERY
  WITH r AS (
    SELECT * FROM public._match_rollups_for_member(target_member_id)
  ),
  agg AS (
    SELECT
      r.opponent_id,
