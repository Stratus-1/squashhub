...
BEGIN
  ...
  IF v_club_id IS NULL AND NEW.challenger_member_id IS NOT NULL THEN
    SELECT cm.club_id INTO v_club_id
    FROM public.club_members cm
    WHERE cm.id = NEW.challenger_member_id
    LIMIT 1;
  END IF;
  IF v_club_id IS NULL AND NEW.challenger_id IS NOT NULL THEN
    SELECT cm.club_id INTO v_club_id
    FROM public.club_members cm
  ...
  -- Resolve challenger rank: prefer member_id lookup, fallback to user_id
  ...
  -- Resolve opponent rank: prefer member_id lookup, fallback to user_id
  ...
  IF max_levels IS NULL THEN max_levels := 2; END IF;

  IF (challenger_rank - opponent_rank) > max_levels THEN
    RAISE EXCEPTION 'You may challenge up to % positions above you', max_levels;
  END IF;

  -- Duplicate check using both member_id and user_id
  IF NEW.challenger_member_id IS NOT NULL AND NEW.opponent_member_id IS NOT NULL THEN
