...
-- Open stats RPCs to all authenticated users (no more privacy gating).
-- Keep auth check; drop privacy_show_advanced_stats / privacy_show_recent_matches enforcement.
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN count(r.duration_s) > 0 THEN round(avg(r.duration_s)::numeric / 60)::int ELSE NULL END,
    max(r.match_date)
  INTO
  ...
  WITH ordered AS (
    SELECT r.is_win, r.match_date, r.created_at,
      row_number() OVER (ORDER BY r.match_date DESC, r.created_at DESC) AS rn,
      CASE WHEN lag(r.is_win) OVER (ORDER BY r.match_date DESC, r.created_at DESC) IS DISTINCT FROM r.is_win THEN 1 ELSE 0 END AS changed
    FROM public._match_rollups_for_player(target_user_id) r
    ORDER BY r.match_date DESC, r.created_at DESC
  ),
  groups AS (SELECT is_win, rn, sum(changed) OVER (ORDER BY rn) AS grp FROM ordered),
  streaks AS (SELECT is_win, grp, count(*)::int AS len FROM groups GROUP BY is_win, grp)
  SELECT
  ...
  RETURN jsonb_build_object(
    'matches', matches_count, 'wins', wins, 'losses', losses,
    'win_rate', win_rate, 'avg_duration_min', avg_duration_min,
    'last_match_date', last_match_date, 'current_streak', current_streak_label,
    'best_win_streak', best_win_streak, 'best_loss_streak', best_loss_streak,
    'sets_for', sets_for, 'sets_against', sets_against,
...
RETURNS TABLE (
  opponent_id uuid, opponent_name text, matches integer, wins integer, losses integer,
  win_rate integer, last_match_date date, avg_duration_min integer,
  sets_for integer, sets_against integer, points_for integer, points_against integer
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  RETURN QUERY
  WITH r AS (SELECT * FROM public._match_rollups_for_player(target_user_id)),
  agg AS (
    ...
    FROM r GROUP BY r.opponent_id
  )
  SELECT a.opponent_id, COALESCE(p.name, 'Unknown') AS opponent_name,
    a.matches, a.wins, a.losses, a.win_rate, a.last_match_date,
    a.avg_duration_min, a.sets_for, a.sets_against, a.points_for, a.points_against
  FROM agg a LEFT JOIN public.profiles p ON p.id = a.opponent_id
...
CREATE OR REPLACE FUNCTION public.get_squash_totals_by_member(target_member_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  INTO
    matches_count, wins, losses, sets_for, sets_against, points_for, points_against, avg_duration_min, last_match_date
  FROM public._match_rollups_for_member(target_member_id) r;
...
REVOKE ALL ON FUNCTION public.get_squash_totals_by_member(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_squash_totals_by_member(uuid) TO authenticated;


CREATE OR REPLACE FUNCTION public.get_head_to_head_by_member(target_member_id uuid, limit_count integer DEFAULT 20)
RETURNS TABLE (
  opponent_id uuid, opponent_name text, matches integer, wins integer, losses integer,
  win_rate integer, last_match_date date, avg_duration_min integer,
  sets_for integer, sets_against integer, points_for integer, points_against integer
...
BEGIN
  ...
  RETURN QUERY
  WITH r AS (SELECT * FROM public._match_rollups_for_member(target_member_id)),
  agg AS (
    SELECT r.opponent_id,
      count(*)::int AS matches,
      count(*) FILTER (WHERE r.is_win)::int AS wins,
...
REVOKE ALL ON FUNCTION public.get_head_to_head_by_member(uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_head_to_head_by_member(uuid, integer) TO authenticated;
