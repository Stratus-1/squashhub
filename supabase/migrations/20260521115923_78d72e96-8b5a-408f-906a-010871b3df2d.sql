...
AS $function$
  WITH target_member AS (
    SELECT
      cm.id,
      lower(regexp_replace(coalesce(cm.name, ''), '\s+', ' ', 'g')) AS member_name
  ...
  league_codes AS (
    SELECT upper(regexp_replace(maa.league_association_number, '[^A-Za-z0-9]', '', 'g')) AS code
    ...
    UNION
    SELECT upper(regexp_replace(mlr.league_association_number, '[^A-Za-z0-9]', '', 'g')) AS code
    FROM public.member_league_registrations mlr
    WHERE mlr.club_member_id = target_member_id
      AND mlr.league_association_number IS NOT NULL
      AND mlr.league_association_number <> ''
  ),
  league_base AS (
    SELECT
      lmr.id AS match_id,
      (CASE WHEN matched.is_home
      ...
      lmr.created_at,
      (lmr.winner = CASE WHEN matched.is_home THEN 'home' ELSE 'away' END) AS is_win,
      NULL::int AS duration_s,
      matched.is_home AS is_player_a,
    ...
    CROSS JOIN target_member tm
    CROSS JOIN LATERAL (
      SELECT CASE
        WHEN upper(regexp_replace(coalesce(lmr.home_player_code, ''), '[^A-Za-z0-9]', '', 'g')) IN (SELECT code FROM league_codes) THEN true
        WHEN upper(regexp_replace(coalesce(lmr.away_player_code, ''), '[^A-Za-z0-9]', '', 'g')) IN (SELECT code FROM league_codes) THEN false
        WHEN lower(regexp_replace(coalesce(lmr.home_player_name, ''), '\s+', ' ', 'g')) = tm.member_name THEN true
        WHEN lower(regexp_replace(coalesce(lmr.away_player_name, ''), '\s+', ' ', 'g')) = tm.member_name THEN false
        ELSE NULL::boolean
      END AS is_home
    ) matched
    WHERE lmr.winner IS NOT NULL
      AND matched.is_home IS NOT NULL
  ),
  league_per_match AS (
    SELECT
      lb.match_id,
