...
BEGIN
  ...
  WHERE mlr.club_member_id = NEW.club_member_id
    ...
    -- Ignore the row being updated and any existing row in the same league.
    -- INSERT ... ON CONFLICT fires this trigger before the conflict update, so
    -- without the league_id exclusion a normal re-save of the same team is
    -- incorrectly treated as a duplicate.
    AND mlr.id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid)
    AND mlr.league_id <> NEW.league_id
