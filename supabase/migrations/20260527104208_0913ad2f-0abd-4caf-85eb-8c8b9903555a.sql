...
BEGIN
  ...
  v_same_club := v_source_league.club_id = v_target_league.club_id;
  IF NOT v_same_club THEN
    RAISE EXCEPTION 'Players can only be moved within the same club';
  END IF;

  -- Authorize by TARGET league: a captain may pull players into their own team pool.
  IF NOT (
  ...
  WHERE lr.association_id = v_target_league.association_id
  LIMIT 1;
  ...
  -- Preserve the old top-down waterfall only for direct same-gender push-down.
