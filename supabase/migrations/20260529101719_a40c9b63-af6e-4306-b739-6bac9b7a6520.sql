...
BEGIN
  ...
  -- Administrative court blocks (league fixtures and club events) are not
  -- personal player bookings and must not consume a member's peak-hour quota.
  IF COALESCE(NEW.source, '') = 'club_event'
  ...
  FOREACH v_mid IN ARRAY v_member_ids LOOP
    ...
    WHERE b.club_id = NEW.club_id
      ...
      AND COALESCE(b.source, '') <> 'club_event'
