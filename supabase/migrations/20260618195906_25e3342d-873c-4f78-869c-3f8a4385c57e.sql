...
-- Back-fill existing delegation_request notifications so old ones also open the dialog
UPDATE public.notifications
   SET url = '/?delegation=' || (data->>'delegation_id')
 WHERE type = 'delegation_request'
   AND data ? 'delegation_id'
   AND (url IS NULL OR url NOT LIKE '%delegation=%');