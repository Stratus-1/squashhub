...
BEGIN
  ...
  -- Administrative / externally-sourced court blocks are not personal player
  -- bookings and must not consume a member's peak-hour quota. This covers
  -- league fixtures, club events, and slots mirrored from the external gobook
  -- provider (which has its own booking limits enforced at source).
  IF COALESCE(NEW.source, '') IN ('club_event', 'gobook')
     OR (NEW.guest_name IS NOT NULL AND (
       NEW.guest_name ~* '\mleague\M'
       OR NEW.guest_name ~* '\mround\s*\d'
       OR NEW.guest_name LIKE '% — %'
     )) THEN
    RETURN NEW;
  END IF;
  ...
  FOREACH v_mid IN ARRAY v_member_ids LOOP
    ...
    WHERE b.club_id = NEW.club_id
      ...
      AND (b.guest_name IS NULL OR (
        b.guest_name !~* '\mleague\M'
        AND b.guest_name !~* '\mround\s*\d'
        AND b.guest_name NOT LIKE '% — %'
      ))
