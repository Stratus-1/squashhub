ALTER TABLE public.stitch_mandates
  ...
  ADD COLUMN IF NOT EXISTS initial_payment_tx_id uuid;
...
BEGIN
  ...
  -- Effective date of the recurring arrangement.
  v_when := COALESCE(m.authorised_at, m.updated_at, now());

  -- Amount taken up front. Subscriptions carry a fixed verification charge
  -- unless a specific initial amount was captured at creation.
  v_total := COALESCE(
    m.initial_amount_cents,
    CASE WHEN m.mandate_type = 'subscription' THEN 2000 ELSE 0 END
  )::numeric / 100.0;
  IF v_total <= 0 THEN RETURN jsonb_build_object('ok', true, 'skipped', 'no_initial_charge'); END IF;

  -- Guard against a duplicate run from a replayed webhook.
  IF EXISTS (
  ...
  -- Allocate against the member's oldest outstanding fees, oldest first, but
  -- only where the payment fully covers the fee (partial fee settlement is not
  -- supported by the fee ledger).
  FOR v_fee IN
    SELECT id, amount
  ...
  LOOP
    ...
    IF v_fee.amount <= v_remaining THEN
      ...
      WHERE id = v_fee.id;
      -- Date the ledger entries on the effective date of the mandate.
  ...
  -- Anything left over is a payment on account (Dr Bank / Cr Debtors via the
  -- existing member-credit trigger).
