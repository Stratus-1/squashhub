CREATE OR REPLACE FUNCTION public.claim_member_by_league_number(
  ...
  _club_member_number text DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Guard: if this league number is already attached to a linked membership at this club,
  -- reuse it (same user) or fail clearly (different user) instead of creating a duplicate.
  IF _club_id IS NOT NULL THEN
    SELECT cm.id, cm.user_id
      INTO v_existing_id, v_existing_user
    ...
    ORDER BY (cm.user_id = v_uid) DESC, cm.joined_at ASC
    LIMIT 1;

    IF v_existing_id IS NOT NULL THEN
      IF v_existing_user = v_uid THEN
        UPDATE public.club_members
        SET email              = COALESCE(email, NULLIF(v_norm_email, '')),
            phone              = COALESCE(phone, NULLIF(btrim(_phone), '')),
            club_member_number = COALESCE(club_member_number, v_norm_member_no),
            plays_league       = true,
            updated_at         = now()
        WHERE id = v_existing_id;
        RETURN v_existing_id;
      ELSE
        RAISE EXCEPTION 'League number % is already linked to another account at this club. Please sign in with that account or contact your club admin.', v_norm_number;
      END IF;
    END IF;
  END IF;
  ...
  IF v_match_id IS NULL THEN
    -- Also reuse an existing membership this user already holds at the club
  ...
  IF v_match_id IS NULL THEN
    INSERT INTO public.club_members (
      ...
      club_member_number, enable_league_association_id
    ) VALUES (
      v_resolved_club_id, v_uid, v_full_name,
      NULLIF(v_norm_email, ''), NULLIF(btrim(_phone), ''),
      true, now(), 'member', v_norm_member_no, v_assoc_id
    )
