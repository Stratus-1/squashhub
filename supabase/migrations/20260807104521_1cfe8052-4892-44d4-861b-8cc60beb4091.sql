CREATE OR REPLACE FUNCTION public.generate_member_renewal_invoices(p_club_id uuid, p_category_ids uuid[] DEFAULT NULL::uuid[], p_league_assoc_ids uuid[] DEFAULT NULL::uuid[], p_national_body_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    -- Skip members who already paid a full year covering this season because
    -- they joined within a month of the renewal date.
    IF v_existing.id IS NULL THEN
      ...
      WHERE club_member_id = r.member_id
        AND season_year = v_season
        AND fee_type <> 'renewal'
        AND fee_label ILIKE '%' || r.source_name || '%'
      LIMIT 1;
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_prepaid', v_skipped_prepaid
  );
END;
$function$;