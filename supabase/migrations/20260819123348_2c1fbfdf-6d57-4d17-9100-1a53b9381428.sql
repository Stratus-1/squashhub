ALTER TABLE public.tournament_governance
  ADD COLUMN IF NOT EXISTS payment_timing text NOT NULL DEFAULT 'on_entry';

ALTER TABLE public.tournament_governance
  DROP CONSTRAINT IF EXISTS tournament_governance_payment_timing_chk;
ALTER TABLE public.tournament_governance
  ADD CONSTRAINT tournament_governance_payment_timing_chk
  CHECK (payment_timing IN ('on_entry','after_acceptance'));

DROP VIEW IF EXISTS public.club_champs;
...
GRANT SELECT ON public.club_champs TO anon;
GRANT ALL ON public.club_champs TO service_role;

CREATE TRIGGER club_champs_instead_insert INSTEAD OF INSERT ON public.club_champs
  FOR EACH ROW EXECUTE FUNCTION public.club_champs_compat_insert();
CREATE TRIGGER club_champs_instead_update INSTEAD OF UPDATE ON public.club_champs
  FOR EACH ROW EXECUTE FUNCTION public.club_champs_compat_update();
CREATE TRIGGER club_champs_instead_delete INSTEAD OF DELETE ON public.club_champs
