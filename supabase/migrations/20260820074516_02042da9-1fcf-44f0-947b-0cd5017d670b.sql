-- 1. New knockout config on the tournament row
ALTER TABLE public.tournaments
  ADD COLUMN IF NOT EXISTS league_sections jsonb NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS knockout_seeds jsonb,
  ADD COLUMN IF NOT EXISTS knockout_seeds_at timestamptz;

COMMENT ON COLUMN public.tournaments.league_sections IS 'Per-league knockout section counts, keyed by group_number as text. e.g. {"1": 2, "2": 4}';
COMMENT ON COLUMN public.tournaments.knockout_seeds IS 'Frozen seeding snapshot per league: {"1":[{"member_id":..,"partner_id":..,"seed":1,"section":1}]}';

-- 2. Section on the match row (knockout draws only)
ALTER TABLE public.club_champs_matches
  ADD COLUMN IF NOT EXISTS section_number integer;

CREATE INDEX IF NOT EXISTS club_champs_matches_ko_idx
  ON public.club_champs_matches (champ_id, group_number, section_number, round_number);

-- 3. Expose the new tournament columns through the club_champs compatibility view
CREATE OR REPLACE VIEW public.club_champs AS
 SELECT t.id,
    ...
    t.day_schedules,
    t.court_ids,
    t.court_rotation_minutes,
    t.group_break_minutes,
    t.default_break_minutes,
    t.invite_source,
    t.invite_include_reserves,
    t.invite_excluded_member_ids,
    t.group_labels,
    t.swiss_pools,
    t.swiss_rounds,
    t.avoid_back_to_back,
    t.schedule_mode,
    t.playoff_break_minutes,
    t.playoff_date,
    t.league_formats,
    t.expected_players,
    t.league_win_conditions,
    g.sanction_status,
    g.sanctioning_org_id,
    g.sanction_reference,
    g.sanction_notes,
    g.sanctioned_at,
    g.sanctioned_by,
    g.competition_level,
    g.eligibility_min_age,
    g.eligibility_max_age,
    g.eligibility_requires_licence,
    g.eligibility_scope,
    g.eligibility_notes,
    g.registration_required,
    g.registration_mode,
    g.registration_opens_at,
    g.registration_closes_at,
    g.entry_fee_cents,
    g.federation_fee_cents,
    g.association_fee_cents,
    g.payment_methods,
    g.payment_required,
    g.refund_policy,
    g.refund_cutoff_date,
    r.scoring_mode,
    r.draw_type,
    r.standard_of_play,
    r.round_format,
    ...
    r.win_condition,
    r.handicap_mode,
    r.handicap_multiplier,
    r.handicap_divider,
    r.bye_handling,
    r.play_all_games,
    r.affects_ranking_points,
    r.no_show_opponent_points,
    r.no_show_player_points,
    g.entry_source,
    g.approval_gate,
    g.payment_timing,
    t.league_sections,
...
-- 4. Compatibility triggers keep writing through the view
...
BEGIN
  ...
  UPDATE public.tournament_governance SET
    ...
    payment_timing = COALESCE(NEW.payment_timing, payment_timing)
