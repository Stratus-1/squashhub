create table if not exists public.tournament_invite_platform_sends (
  id uuid primary key default gen_random_uuid(),
  registration_id uuid not null references public.club_champs_registrations(id) on delete cascade,
...
grant select on public.tournament_invite_platform_sends to authenticated;
grant all on public.tournament_invite_platform_sends to service_role;

alter table public.tournament_invite_platform_sends enable row level security;

drop policy if exists "admins view platform invite sends" on public.tournament_invite_platform_sends;
create policy "admins view platform invite sends"
on public.tournament_invite_platform_sends
...
using (
  exists (
    ...
    where t.id = tournament_invite_platform_sends.champ_id
