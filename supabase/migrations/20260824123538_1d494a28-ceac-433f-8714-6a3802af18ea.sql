ALTER TABLE public.tournaments ADD COLUMN IF NOT EXISTS pool_allocation text NOT NULL DEFAULT 'snake';
...
CREATE OR REPLACE VIEW public.club_champs AS
 SELECT t.id, t.club_id, t.owner_org_id, t.name, t.gender, t.status, t.num_groups,
    ...
    t.champion_scope, t.pool_sizes, t.pool_allocation
   FROM tournaments t
...
BEGIN
  UPDATE public.tournaments SET
    ...
    pool_sizes = COALESCE(NEW.pool_sizes, pool_sizes),
