...
BEGIN
  ...
  v_url := '/club-champs/' || p_champ_id::text;
  ...
  LOOP
    ...
    IF COALESCE(m.is_bye, false) OR m.a IS NULL OR m.b IS NULL THEN
      -- Bye: tell the player they advance, no opponent contact needed.
    ...
    -- Player A -> opponent B, and Player B -> opponent A
    FOR i IN 1..2 LOOP
      DECLARE
        v_to uuid;
        v_opp text;
      ...
      BEGIN
        ...
        v_msg := v_name || ' — ' || v_round_label || ': you play ' || v_opp
              ...
              || '. Please contact your opponent to schedule and book a court before '
