-- 1) Pyramid ladders default to insert movement (no swap)
UPDATE public.ladder_configs
SET movement_policy = 'insert', updated_at = now()
WHERE format = 'pyramid' AND movement_policy <> 'insert';

-- 2) Challenge confirmation honors the club's movement_policy
...
BEGIN
  ...
  IF NEW.challenge_id IS NOT NULL THEN
    ...
    -- Club's configured movement policy (default: insert / take-the-spot)
    SELECT lc.movement_policy INTO v_movement
    FROM public.ladder_configs lc
    WHERE lc.club_id = v_club_id;
    v_movement := COALESCE(v_movement, 'insert');

    -- Only move the ladder when the CHALLENGER (lower-ranked player) wins.
    IF FOUND AND (
      ...
      IF challenger_rank IS NOT NULL
        ...
        IF v_movement = 'swap' THEN
          -- Swap: the two players exchange positions, nobody else moves.
        ...
        ELSE
          -- Insert (default, pyramid-style): winner takes the spot,
          -- everyone between (incl. loser) shifts down one.
