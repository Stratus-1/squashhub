-- Add a quick-register link to tournament invitation emails for members who
-- have not yet claimed a SquashHub account.
...
begin
  ...
  loop
    v_signup_url := 'https://' || p_club_host || '/auth?intent=claim&redirectTo=/i/' || r.invite_token;
...
GRANT EXECUTE ON FUNCTION public.send_tournament_invites_via_platform(uuid, integer, text, text, text, text, text) TO authenticated;
