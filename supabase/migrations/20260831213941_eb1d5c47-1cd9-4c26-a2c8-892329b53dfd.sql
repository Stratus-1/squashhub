...
declare
  ...
  v_message text;
begin
  select c.logo_url, t.invite_extra_details
    into v_club_logo, v_extra
    from public.club_champs t
    join public.clubs c on c.id = t.club_id
   where t.id = p_champ_id;

  -- Extra details come first, followed by the standard invitation text.
  -- Double newlines separate paragraphs; the template renders them as distinct blocks.
  v_message := coalesce(nullif(trim(v_extra), '') || chr(10) || chr(10), '') ||
    'You are invited to enter ' || p_tournament_name || '.' || chr(10) ||
    ...
    perform net.http_post(
      ...
      body := jsonb_build_object(
        ...
        'templateData', jsonb_build_object(
          ...
          'clubLogoUrl', coalesce(v_club_logo, ''),
          'title', 'Tournament invitation: ' || p_tournament_name,
          'recipientName', r.name,
          'messageBody', v_message,
