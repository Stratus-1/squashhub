...
VALUES ('device_schedule_internal_secret', encode(gen_random_bytes(32), 'hex'))
...
BEGIN
  SELECT value INTO v_secret
    ...
    WHERE key = 'device_schedule_internal_secret';

  PERFORM cron.unschedule('device-schedule-runner-5m')
    WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'device-schedule-runner-5m');

  PERFORM cron.schedule(
    'device-schedule-runner-5m',
    '*/5 * * * *',
    format($cron$
      SELECT net.http_post(
        url := 'https://bzbuppwzljadulwntjys.supabase.co/functions/v1/device-schedule-runner',
