alter table public.clubs
  add column if not exists ranking_provisional_enabled boolean not null default true,
  add column if not exists ranking_provisional_start_points numeric not null default 600,
  add column if not exists ranking_provisional_min_matches integer not null default 5;

alter table public.association_ranking_settings
  add column if not exists provisional_enabled boolean not null default true,
  add column if not exists provisional_start_points numeric not null default 600,
  add column if not exists provisional_min_matches integer not null default 5;
...
alter table public.tournament_rules
  add column if not exists ranking_scope text;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'tournament_rules_ranking_scope_chk') then
    alter table public.tournament_rules
      add constraint tournament_rules_ranking_scope_chk
      check (ranking_scope is null or ranking_scope in ('club','association','national')) not valid;
  end if;
end $$;
