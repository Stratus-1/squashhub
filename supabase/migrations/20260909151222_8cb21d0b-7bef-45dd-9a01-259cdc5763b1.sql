...
BEGIN
  ...
  league_dates AS (
    SELECT played_on FROM league
    UNION
    SELECT played_on FROM nsa
  ),
  club_matches AS (
    SELECT
    ...
    WHERE (m.player_a_member_id = _member_id OR m.player_b_member_id = _member_id)
      ...
      -- An authoritative league rubber already covers this date: don't let the
      -- imported copy of the same night also count as a club match.
      AND NOT (
        COALESCE(m.is_imported, false)
        AND EXISTS (SELECT 1 FROM league_dates ld WHERE ld.played_on = m.match_date)
      )
  ),
  champ AS (
    SELECT
