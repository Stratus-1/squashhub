...
BEGIN
  ...
  WITH league AS (
    SELECT
      'league'::text AS s_source,
      r.id AS s_match_id,
      'league'::text AS s_category,
      EXTRACT(YEAR FROM f.fixture_date)::int AS s_season_year,
      f.fixture_date AS s_played_on,
      CASE WHEN r.home_player_member_id = _member_id THEN r.away_player_member_id
           ELSE r.home_player_member_id END AS s_opponent_member_id,
      COALESCE(NULLIF(f.division, ''), 'League') AS s_event_label,
      COALESCE(r.home_games_won::text || '-' || r.away_games_won::text, '') AS s_score,
      CASE
        WHEN r.home_player_member_id = _member_id OR r.home_player2_member_id = _member_id THEN r.winner = 'home'
        ELSE r.winner = 'away'
      END AS s_won,
      NULL::text AS s_opponent_fallback
  ...
  SELECT
    ...
    COALESCE(om.name, u.s_opponent_fallback, 'Unknown') AS o_name,
    u.s_event_label,
    u.s_score,
    u.s_won
  FROM unioned u
