...
-- Mark rows created by the auto-seeder
...
BEGIN
  ...
  IF v_member.fee_category_id IS NOT NULL THEN
    ...
    -- Never seed a membership fee when one already exists for this season
    -- (e.g. a pro-rated joining fee raised by the onboarding wizard).
  ...
  IF COALESCE(v_member.plays_league, false) THEN
    ...
    INSERT INTO public.club_member_fee_payments
      (club_member_id, fee_type, fee_label, amount, paid, paid_at, season_year, is_pass_through, auto_seeded)
    VALUES
      (p_club_member_id, 'association', COALESCE(v_nsa_assoc.abbreviation, 'NSA'),
       v_nsa_amount, true, now(), v_year, true, true)
    ON CONFLICT (club_member_id, fee_type, fee_label, season_year) DO NOTHING;
    ...
    INSERT INTO public.club_member_fee_payments
      (club_member_id, fee_type, fee_label, amount, paid, paid_at, season_year, is_pass_through, auto_seeded)
    VALUES
      (p_club_member_id, 'national', 'SSA', v_ssa_amount, true, now(), v_year, true, true)
    ON CONFLICT (club_member_id, fee_type, fee_label, season_year) DO NOTHING;
  END IF;
  ...
  LOOP
    ...
    VALUES
      (p_club_member_id,
       ...
       true, now(), v_year, true, true)
    ON CONFLICT (club_member_id, fee_type, fee_label, season_year) DO NOTHING;
  END LOOP;
END;
$$;

-- A real membership fee replaces any auto-seeded one for the same season
CREATE OR REPLACE FUNCTION public.replace_auto_seeded_membership_fee()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF NEW.fee_type = 'club' AND NOT COALESCE(NEW.auto_seeded, false) THEN
    DELETE FROM public.club_member_fee_payments f
     WHERE f.club_member_id = NEW.club_member_id
       AND f.fee_type = 'club'
       AND f.season_year = NEW.season_year
       AND f.id <> NEW.id
       AND f.auto_seeded = true;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_replace_auto_seeded_membership_fee ON public.club_member_fee_payments;
CREATE TRIGGER trg_replace_auto_seeded_membership_fee
AFTER INSERT ON public.club_member_fee_payments
FOR EACH ROW EXECUTE FUNCTION public.replace_auto_seeded_membership_fee();

-- Clean up existing duplicates: keep the non-seeded (real) membership fee
DELETE FROM public.club_member_fee_payments f
WHERE f.fee_type = 'club'
  AND f.paid = true
  AND f.paid_at IS NOT NULL
  AND f.fee_label LIKE 'Club – %'
  AND EXISTS (
    SELECT 1 FROM public.club_member_fee_payments g
    WHERE g.club_member_id = f.club_member_id
      AND g.fee_type = 'club'
      AND g.season_year = f.season_year
      AND g.id <> f.id
      AND g.fee_label NOT LIKE 'Club – %'
  );