  ...
  SELECT COALESCE(jsonb_agg(x ORDER BY x ->> 'display_name'), '[]'::jsonb) INTO v_rows FROM (
    SELECT jsonb_build_object(
     ...
     WHERE r.champ_id = p_champ_id
       ...
       -- Only people who are really in this tournament: they either picked this
       -- doubles division, or they were actually invited / already accepted.
       -- Bulk admin-seeded rows with no invite and no division choice are not
       -- selectable partners.
       AND (
             p_group_number = ANY (COALESCE(r.division_choices, '{}'))
             OR (
               COALESCE(array_length(r.division_choices, 1), 0) = 0
