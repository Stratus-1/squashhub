...
BEGIN
  ...
  -- SMS is billed per 160 GSM characters, and a single non-GSM character
  -- (e.g. an em dash) halves that to 70. The SMS wording below is therefore
  -- kept plain ASCII and short so one notice = one billed message.
  ...
  LOOP
    ...
    -- Knockout milestone rounds (quarterfinal / semifinal / final) get a
    -- congratulation line ahead of the match details. The SMS variant stays
    -- short and plain ASCII so it remains one billed message.
    v_praise := NULL;
    v_praise_sms := NULL;
    ...
    FOR i IN 1..2 LOOP
      ...
      BEGIN
        ...
        v_opp_short := left(regexp_replace(v_opp, '[^A-Za-z0-9 ]', '', 'g'), 24);
