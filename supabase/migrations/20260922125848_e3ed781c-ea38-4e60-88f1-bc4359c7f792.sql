DROP FUNCTION IF EXISTS public.get_member_match_history(uuid, integer, text, uuid);
...
BEGIN
  ...
  champ (s_source, s_match_id, s_category, s_season_year, s_played_on, s_opponent_member_id, s_partner_member_id, s_opponent2_member_id, s_event_label, s_score, s_won, s_opponent_fallback) AS (
    SELECT
      ...
      END,
      -- the member's own partner on this court
      CASE
...
REVOKE EXECUTE ON FUNCTION public.get_member_match_history(uuid, integer, text, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_member_match_history(uuid, integer, text, uuid) TO authenticated, service_role;