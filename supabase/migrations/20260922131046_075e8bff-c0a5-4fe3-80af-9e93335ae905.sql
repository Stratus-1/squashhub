CREATE OR REPLACE FUNCTION public.get_member_match_history(_member_id uuid, _season_year integer DEFAULT NULL::integer, _category text DEFAULT NULL::text, _opponent_member_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(source text, match_id uuid, category text, season_year integer, played_on date, opponent_member_id uuid, opponent_name text, partner_member_id uuid, partner_name text, opponent2_member_id uuid, opponent2_name text, event_label text, score text, won boolean)
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN QUERY
  WITH league (s_source, s_match_id, s_category, s_season_year, s_played_on, s_opponent_member_id, s_partner_member_id, s_opponent2_member_id, s_event_label, s_score, s_won, s_opponent_fallback) AS (
    SELECT
      'league'::text,
      r.id,
      'league'::text,
      ...
      f.fixture_date,
      CASE WHEN r.home_player_member_id = _member_id OR r.home_player2_member_id = _member_id
             THEN r.away_player_member_id
           ELSE r.home_player_member_id END,
      CASE WHEN r.home_player_member_id = _member_id THEN r.home_player2_member_id
           WHEN r.home_player2_member_id = _member_id THEN r.home_player_member_id
           WHEN r.away_player_member_id = _member_id THEN r.away_player2_member_id
           WHEN r.away_player2_member_id = _member_id THEN r.away_player_member_id
      END,
      CASE WHEN r.home_player_member_id = _member_id OR r.home_player2_member_id = _member_id
             THEN r.away_player2_member_id
           ELSE r.home_player2_member_id END,
  ...
  nsa (s_source, s_match_id, s_category, s_season_year, s_played_on, s_opponent_member_id, s_partner_member_id, s_opponent2_member_id, s_event_label, s_score, s_won, s_opponent_fallback) AS (
    SELECT
      'nsa'::text,
      h.id,
      'league'::text,
  ...
  club_matches (s_source, s_match_id, s_category, s_season_year, s_played_on, s_opponent_member_id, s_partner_member_id, s_opponent2_member_id, s_event_label, s_score, s_won, s_opponent_fallback) AS (
    SELECT
      'match'::text,
      m.id,
    ...
    WHERE (m.player_a_member_id = _member_id OR m.player_b_member_id = _member_id)
      ...
      -- Skip the flat singles copy of a doubles tournament game: the same result
      -- is already returned from club_champs_matches with all four players named.
      AND NOT EXISTS (
        SELECT 1
        FROM public.club_champs_matches cm
        WHERE cm.status = 'completed'
          AND COALESCE(cm.scheduled_date, cm.created_at::date) = m.match_date
          AND COALESCE(cm.score, '') = COALESCE(m.score, '')
          AND (cm.partner_a_member_id IS NOT NULL OR cm.partner_b_member_id IS NOT NULL)
          AND m.player_a_member_id IN (cm.player_a_member_id, cm.player_b_member_id, cm.partner_a_member_id, cm.partner_b_member_id)
          AND m.player_b_member_id IN (cm.player_a_member_id, cm.player_b_member_id, cm.partner_a_member_id, cm.partner_b_member_id)
      )
  ),
  champ (s_source, s_match_id, s_category, s_season_year, s_played_on, s_opponent_member_id, s_partner_member_id, s_opponent2_member_id, s_event_label, s_score, s_won, s_opponent_fallback) AS (
    SELECT
      'champ'::text,
      cm.id,
      ...
      END,
      CASE
        WHEN cm.player_a_member_id = _member_id THEN cm.partner_a_member_id
        WHEN cm.partner_a_member_id = _member_id THEN cm.player_a_member_id
        WHEN cm.player_b_member_id = _member_id THEN cm.partner_b_member_id
        WHEN cm.partner_b_member_id = _member_id THEN cm.player_b_member_id
      END,
      CASE
        WHEN cm.player_a_member_id = _member_id OR cm.partner_a_member_id = _member_id
          THEN cm.partner_b_member_id
        ELSE cm.partner_a_member_id
      END,
  ...
  SELECT
    ...
    u.s_partner_member_id,
    pm.name,
    u.s_opponent2_member_id,
    o2.name,
    u.s_event_label,
    u.s_score,
    u.s_won
  ...
  LEFT JOIN public.club_members pm ON pm.id = u.s_partner_member_id
  LEFT JOIN public.club_members o2 ON o2.id = u.s_opponent2_member_id
  WHERE (_season_year IS NULL OR u.s_season_year = _season_year)
    ...
    AND (
      _opponent_member_id IS NULL
      OR u.s_opponent_member_id = _opponent_member_id
      OR u.s_opponent2_member_id = _opponent_member_id
    )
